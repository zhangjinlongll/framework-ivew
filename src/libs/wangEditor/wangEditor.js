(function (global, factory) {
    ...
    */
    ...
    var config = {
        ...
        // 默认菜单配置
        menus: ['head', 'bold', 'italic', 'underline', 'strikeThrough', 'foreColor', 'backColor', 'link', 'list', 'justify', 'quote', 'emoticon', 'image', 'table', 'video', 'code', 'undo', 'redo','staricon'],
    ...
    */
    ...
    DropList.prototype = {
        ...
        show: function show() {
            ...
            // 解决移动端下拉框被遮挡
            try {
                if ($menuELem[0].parentNode && $menuELem[0].style['zIndex']) {
                    var _zIndex = parseInt($menuELem[0].style['zIndex']);
                    var _maxIndex = $menuELem[0].parentNode.getAttribute("maxIndex") ? parseInt($menuELem[0].parentNode.getAttribute("maxIndex")) : 0
                    if (_zIndex >= _maxIndex) {
                        $menuELem[0].parentNode.setAttribute("maxIndex", _zIndex)
                        _maxIndex = _zIndex
                    }
                    $menuELem.css('z-index', _maxIndex + 1);
                }
            }
            catch(err) {}
            if (this._rendered) {
    ...
    */
    // 构造函数
    function Staricon(editor) {
        this.editor = editor;
        this.$elem = $('<div class="w-e-menu" style="font-size: 16px;padding-top: 2px;">\n            <i class="w-e-icon-staricon"><i/>\n        </div>');
        this.type = 'click';
    ...
    // 原型
    Staricon.prototype = {
        constructor: Staricon,
        // 点击事件
        onClick: function onClick(e) {
            var editor = this.editor;
            editor.cmd.do('insertHTML', '<span>★</span>');
        }
    };
    /*
        menu - table
    ...
    */
    ...
    Image.prototype = {
        ...
        _createInsertPanel: function _createInsertPanel() {
            ...
            var addBtnId = getRandom('add-btn');
            var iframeUrl ='/#/chooseImage';
            ...
            function receiveMsg(event)
            {
              ...
              if (origin == host && event.data && event.data.action == 'editorAddImage'){
                urlList = event.data.message
              }
            }
            ...
            var tabsConfig = [{
                title: '上传图片',
                tpl: '<div class="w-e-up-img-container" style="margin-top: 40px;">\n                    <div id="' + upTriggerId + '" class="w-e-up-btn" style="ont-size: 100px;">\n                        <i class="w-e-icon-upload2"></i>\n                    </div>\n                    <div style="display:none;">\n                        <input id="' + upFileId + '" type="file" multiple="multiple" accept="image/jpg,image/jpeg,image/png,image/gif,image/bmp"/>\n                    </div>\n                </div>',
                events: [{
                    // 触发选择图片
            ...
            }, // second tab end
            {
                // 选择图库图片
                title: '选择图片',
                tpl: '<div style="height:100%"><iframe src="'+ iframeUrl +'" style="border:0;width:100%;height:100%;background-color: #ffffff;"></iframe><div class="w-e-button-container"><button id="' + addBtnId + '" class="right" style="position: absolute;bottom: 20px;right: 30px;color: #fff;background-color: #2d8cf0;border-color: #2d8cf0;">\u63D2\u5165</button></div></div>',
                events: [{
                    selector: '#' + addBtnId,
                    type: 'click',
                    fn: function fn() {
                        ...
                        if (urlList.length > 0) {
                            urlList.forEach(function (item, i) {
                                uploadImg.insertLinkImg(item + '?x-oss-process=image/resize,m_fill,h_300,w_400');
                            })
                        }
                        // 返回 true 表示函数执行结束之后关闭 panel
                        return true;
    ...
    */
    ...
    MenuConstructors.staricon = Staricon;

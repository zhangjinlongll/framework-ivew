<template>
  <div :class="classes" v-clickoutside="handleClose">
    ...
    <transition name="slide-up">
      <div class="ivu-select-dropdown cascader-multi" v-show="visible" :class="{ [prefixCls + '-transfer']: transfer,  'full-width': fullwidth}" ref="drop" :data-transfer="transfer" v-transfer-dom v-if="!destroy">
        <div>
          <cas-multi-panel @handleClose="handleClose" :value="queryItem" v-if="(data.length && (!filterable || query === ''))" @handleGetSelected="selectedData" @clearQueryItem="queryItem = []" :data="data" :multiple="multiple" :fullwidth="fullwidth" :fontsize="fontsize"></cas-multi-panel>
  ...
  methods: {
    ...
    handleSelectItem(index) {
      ...
      if (itemvalue.indexOf(',') !=-1){
        ...
        getSelectItem(this.data, [item.value], temp_data);
      }
      this.queryItem = temp_data;
      let isMultiple = false;
      this.queryItem.map(function(n){
        if(n.multiple) isMultiple = true
      })
      if(isMultiple) {
        let _this = this
        let nvalue = JSON.parse(JSON.stringify(this.value))
        this.queryItem.map(function(n){
          nvalue.push(n.value)
        })
        let arr = new Set(nvalue)
        this.setMultiData(Array.from(arr))
      }else{
